...
class AppState(rx.State):
    ...
    transactions_json: str = rx.LocalStorage("[]", name="transactions_v1")
    ...
    def add_transaction(self, form_data: dict):
        ...
        try:
            ...
            self.form_error = "Invalid amount."
            return
    ...
    def pending_payables(self) -> float:
        ...
        return sum((t.amount for t in self.transactions if t.type == "Payables"))

    @rx.var
    def pending_receivables(self) -> float:
        """Calculates the total amount of pending receivables."""
        return sum((t.amount for t in self.transactions if t.type == "Receivables"))